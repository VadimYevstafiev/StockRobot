function AddbidsJournal(input) {
  var string = '';
  string += '<table id = "protocolstable">';
  string += '<tr><th>Время запуска</th><th>Результат запуска</th><th>Текст протокола</th></tr>';

}


function AddHead(input) {
  var output = '<tr>';
   for (key in input) {
     output += '<th style="width:' + input[key][0] + 'px;">' + input[key][1] + '</th>';
  }
  output += '</tr>';
  return output;
}
...
function AddCharts(radioinput, boxinput, data) {
  ...
  var size;
  var basesize = 1700;
  var sizemod = [[6, 2], [0, 0], [0, 0], [6, 2]];
  size = basesize * sizemod[timefame][radioinput[1]];
